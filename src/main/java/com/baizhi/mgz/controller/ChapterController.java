...
import com.baizhi.mgz.service.ChapterService;
...
public class ChapterController {
    ...
    public Map showChapterById(Integer page,Integer rows,String albumId){
        ...
        int i = chapterService.selectCount(chapter);
        Integer total = i%rows==0?i/rows:i/rows+1;
        List<Chapter> chapterDaos = chapterService.ShowAllChapters(chapter,page , rows);
    ...
    public Map editChapter(String oper, Chapter chapter, String[] id,String albumId,HttpSession session){
        HashMap hashMap = new HashMap();
        // 添加逻辑
        if (oper.equals("add")) {
    ...
    public Map uploadChapter(MultipartFile url, String chapterId, HttpSession session, HttpServletRequest request) throws TagException, ReadOnlyFileException, CannotReadException, InvalidAudioFrameException, IOException {
        ...
        Chapter chapter = new Chapter();
        String chapterId1 = (String) session.getAttribute("chapterId");
